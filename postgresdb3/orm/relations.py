...
class ReverseRelation:
    ...
    def __set__(self, instance, value):
        raise AttributeError(
            "Teskari bog'lanishni to'g'ridan-to'g'ri o'zgartirib bo'lmaydi."
        )
...
class ManyToManyRelation:
    ...
    def __init__(
        self, target_model, through_table, source_col, target_col, is_async=False
    ):
    ...
    def __get__(self, instance, owner):
        ...
        qs = self.target_model.filter(
            **{f"{self.through_table}.{self.source_col}": pk_value}
        )
    ...
    def __set__(self, instance, value):
        raise AttributeError(
            "ManyToMany bog'lanishni to'g'ridan-to'g'ri o'zgartirib bo'lmaydi. .add() yoki .remove() ishlating."
        )
